...
export class ProcessListenerLinux extends EventEmitter {
	...
	public on<K extends keyof LinuxEvents>(e: K, listener: LinuxEvents[K]): this { return super.on(e, listener); }
  	public emit<K extends keyof LinuxEvents>(event: K, ...args: Parameters<LinuxEvents[K]>): boolean {return super.emit(event, ...args)}
	...
	lastActiveWindow() {
		...
		return [...this.windowDetails.values()]
			...
			.map(details => {
				const window = windows.find(w => w.owner.path === details.path)! as LinuxResultExtra;
				if (!window) return undefined;
				window.createdAt = details.createdAt;
				window.lastFocusAt = details.lastFocusAt;
				return window
	...
	loop() {
		...
		if (newLastActiveWindow?.owner.path !== this.lastActiveWindowCache?.owner.path) {
			this.lastActiveWindowCache = newLastActiveWindow;
			this.emit("change", newLastActiveWindow);
		}
	}
	private updateWindowDetails(window: LinuxResultExtra, forceUpdateFocusAt?: boolean) {
		const obj = {...this.windowDetails.get(window.owner.path) || {path: window.owner.path, createdAt: Date.now()}} as WindowDetails;
		(forceUpdateFocusAt || !obj.lastFocusAt) && (obj.lastFocusAt = Date.now());
